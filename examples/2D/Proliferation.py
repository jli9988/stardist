...
# --- USER PARAMETERS ---
input_folder   = r"J:/Cells/CellTrace"
pattern        = "*.czi"
min_cell_area  = 50
min_foci_area  = 2
# -----------------------
...
for fp in sorted(glob(os.path.join(input_folder, pattern))):
    ...
    red    = img[0]    # First channel
    green  = img[3]    # Fourth channel
    orange = img[1]    # Second channel
    bf     = img[1]    # Third channel

    # Apply Gaussian smoothing before normalization
    sigma = 4
    red_smooth    = gaussian(red,    sigma=1, preserve_range=True)
    orange_smooth = gaussian(orange, sigma=sigma, preserve_range=True)
